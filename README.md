================================================================
  HiddenTrails.AI — Project README
  AI-Powered Eco-Travel Platform
...
* Admin panel to manage places (requires ADMIN_KEY)
...
It should already be present and contain:
    ...
    ADMIN_KEY=hiddentrails_admin_2026
...
It requires the ADMIN_KEY from backend/.env to be passed as
a request header:

    x-admin-key: hiddentrails_admin_2026

Use this panel to add, edit, or delete places in the system.
...
* Frontend URL   : http://localhost:5000 (served by backend)
* Active internet connection is required at runtime
  (Firebase services + Groq AI API calls)
* The backend uses ES Module syntax (import/export),
  NOT CommonJS (require). Do not mix the two.
* Firebase Firestore is the sole database — no SQL or
  local database setup needed.
* All AI calls go to api.groq.com (external service).
...
-------------------------------------------------------------------
  ...
  [ ] Project zip extracted to desktop
  [ ] backend/config/serviceAccountKey.json is in place
  [ ] backend/.env contains GROQ_API_KEY and ADMIN_KEY
  [ ] Open terminal → cd HiddenTrails.AI/backend
  [ ] Run: npm install
  [ ] Run: node server.js
  [ ] Open browser → http://localhost:5000
  [ ] Health check at http://localhost:5000/health shows healthy
